...
if (empty($fname)) {
    ...
    $r = Database::search("SELECT * FROM `student` WHERE `email` = '" . $email . "' OR `name` = '" . $username . "' ");
    if ($r->num_rows > 0) {
        echo "User with the same email/name already exist";
    } else {
        $d = new DateTime();
        ...
        Database::iud("INSERT INTO `online_student_management_system`.`student` (`name`, `birthday`,`email`,`grade_id`,`registration_date`,`password`) 
